...
export default function DriversPage() {
  ...
  const { data: expiring } = useQuery({
    ...
    queryFn: async () => (await api.get("/drivers/expiring/", { params: { days: 30 } })).data,
  });
  ...
  return (
    <Layout>
      ...
      {expiring && expiring.count > 0 && (
        <div className="mb-4 flex items-start gap-3 rounded-lg border border-orange-300 bg-orange-50 px-4 py-3 text-sm dark:border-orange-900/60 dark:bg-orange-950/30">
          <AlertTriangle size={18} className="mt-0.5 shrink-0 text-orange-500" />
          <div>
            <div className="font-medium text-orange-700 dark:text-orange-300">
              {expiring.count} driver license{expiring.count > 1 ? "s" : ""} expiring within 30 days
            </div>
            <div className="mt-1 text-orange-600 dark:text-orange-400/80">
              {expiring.drivers.map((d) => `${d.name} (${d.license_expiry})`).join(" · ")}
            </div>
            <div className="mt-1 text-xs text-orange-500/80 dark:text-orange-400/60">
              Automated email reminders: run <code>python manage.py notify_expiring_licenses</code> (schedule via cron).
            </div>
          </div>
        </div>
      )}
